...
test("doctor JSON contract deterministically prioritizes invalid configuration", () => {
  ...
  const result = spawnSync(
    process.execPath,
    ["--import", "tsx", "scripts/doctor.ts", "--json", "--profile", "__doctor_contract_invalid_profile__", "--data-dir", missingDataDir],
  ...
  assert.equal(report.checks.some((check) => check.id === "storage.sqlite_integrity"), false, "doctor must not run profile integrity against an invalid profile");
});
...
test("doctor human output names failing checks and recovery actions", () => {
  ...
  const result = spawnSync(process.execPath, ["--import", "tsx", "scripts/doctor.ts", "--profile", "__doctor_contract_invalid_profile__", "--data-dir", missingDataDir], { cwd: process.cwd(), encoding: "utf8" });
  assert.equal(result.signal, null, result.stderr);
  assert.equal(result.status, 3, result.stderr);
...
test("doctor output never exposes configured secret values", () => {
  ...
  const result = spawnSync(process.execPath, ["--import", "tsx", "scripts/doctor.ts", "--json", "--profile", "__doctor_contract_invalid_profile__", "--data-dir", missingDataDir], { cwd: process.cwd(), encoding: "utf8", env: { ...process.env, MW_SESSION_SECRET: secret, MW_BOOTSTRAP_PASSWORD: secret } });
  assert.equal(result.signal, null, result.stderr);
  assert.equal(result.status, 3, result.stderr);
...
test("doctor does not mutate the inspected data directory on invalid configuration", () => {
  ...
  try {
    const result = spawnSync(process.execPath, ["--import", "tsx", "scripts/doctor.ts", "--json", "--profile", "__doctor_contract_invalid_profile__", "--data-dir", dataDir], { cwd: process.cwd(), encoding: "utf8" });
    assert.equal(result.signal, null, result.stderr);
    assert.equal(result.status, 3, result.stderr);
...
test("doctor classifies missing plugin lock as invalid configuration", () => {
  ...
  try {
    const result = spawnSync(process.execPath, ["--import", tsxImport, doctorScript, "--json", "--profile", "__doctor_contract_invalid_profile__", "--data-dir", missingDataDir], { cwd: isolatedCwd, encoding: "utf8" });
    assert.equal(result.signal, null, result.stderr);
    assert.equal(result.status, 3, result.stderr);
    const report = JSON.parse(result.stdout) as DoctorReport;
    const pluginLock = report.checks.find((check) => check.id === "configuration.plugin_lock");
    ...
    assert.equal(pluginLock?.recovery, "Run pnpm plugins:lock:write and review the resulting lock diff before startup.");
  } finally {
...
test("doctor classifies readable but uninitialized profile storage as runtime failure", () => {
  ...
  try {
    const result = spawnSync(process.execPath, ["--import", "tsx", "scripts/doctor.ts", "--json", "--profile", "standalone-business", "--data-dir", dataDir], { cwd: process.cwd(), encoding: "utf8" });
    assert.equal(result.signal, null, result.stderr);
    ...
    assert.equal(integrity?.recovery, "Inspect the reported domain databases and restore or repair corrupted SQLite state before startup.");
  } finally {
    rmSync(dataDir, { recursive: true, force: true });
  }
});
