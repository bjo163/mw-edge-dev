...
const TYPES = new Set(["String","Text","Integer","Decimal","Boolean","DateTime","Enum","Reference","Relation","Json"]);
export class ModelRegistry {
  #models = new Map();
  register(model, owner) {
    ...
    for (const [name, field] of Object.entries(model.fields || {})) {
      ...
      if (field.type === "Enum" && (!Array.isArray(field.enum) || field.enum.length === 0)) throw new Error(`Enum values required for ${model.name}.${name}`);
      if (field.type === "Relation" && !field.target) throw new Error(`Relation target required for ${model.name}.${name}`);
    }
  ...
  finalize() {
    for (const model of this.#models.values()) {
      for (const [fieldName, field] of Object.entries(model.fields)) {
        ...
        if (target.domain !== model.domain) throw new Error(`Cross-domain Relation forbidden: ${model.name}.${fieldName} -> ${field.target}`);
        if (!target.fields[targetField]) throw new Error(`Missing relation field ${field.target}`);
      }
    }
    return this;
  }
  get(name) { const m = this.#models.get(name); if (!m) throw new Error(`Unknown model ${name}`); return m; }
